...
public class ZCSE_2447_CombinedCheck {
	...
	String fname = "test";
	String lname = "tbi";
	String email = "tbicombine" + number +"@tbi.com";
	String pwd = "Test@123";
	...
	public void userSignedIn() throws InterruptedException {
		...
		extent.attachReporter(repo);
		ExtentTest logger = extent.createTest("Zineone Setup and User Login Check");
	...
	public void TBI_TestGroupCheckLoggedin() throws InterruptedException {
		extent.attachReporter(repo);
		ExtentTest logger = extent.createTest("TBI Test Group check - Logged In");
		if (connectData == true && isLoggedIn == true) {
			// Home page
			d.findElement(By.xpath("//*[@id=\"main-header\"]/div[7]/section/a")).click();
			Thread.sleep(10000);
			for (int i = 0; i <= 1; i++) {
			...
			logger.log(Status.FAIL, "Not Logged In");
			result = false;
			Assert.assertTrue(result);
			extent.flush();
		}
	...
	public void TBI_ControlGroupCheckLoggedin() throws InterruptedException {
		extent.attachReporter(repo);
		ExtentTest logger = extent.createTest("TBI Control Group check - Logged In");
		if (connectData == true && isLoggedIn == true) {
	...
	public void TBI_CopyClickCheckLoggedin() throws InterruptedException {
		extent.attachReporter(repo);
		ExtentTest logger = extent.createTest("TBI Offer Code Copy Check - Logged In");
		if (connectData == true && isLoggedIn == true) {
			Actions a = new Actions(d);
			// copying coupen code
			d.findElement(By.xpath("//*[@id=\"btn_addoffer\"]")).click();
